...
const statusBorder: Record<Exclude<LeaveStatus, null>, string> = {
...
// Year view only for now

const CalendarYearView = () => {
  const { view, date, today, locale, leaveRequests, leaveTypes } = useCalendar()
  ...
  const getLeaveTypeColorForDate = (d: Date): string | null => {
    ...
    if (req.leaveTypeCode === 'SICK') return '#3498DB'
    return null
  }
      ...
      {months.map((days, i) => (
        <div key={days[0].toString()}>
          ...
          <div className="grid gap-x-0 gap-y-1 text-center grid-cols-7 text-xs tabular-nums">
            {days.map(_date => {
              const status = getLeaveStatusForDate(_date)
              const typeColor = getLeaveTypeColorForDate(_date)
              ...
              return (
                ...
                >
                  <div
                    className={cn(
                      'relative aspect-square grid place-content-center size-full tabular-nums',
